# Overview
 Crystialized water floating down as snow is like DNA while raindrops are fairly distinct depending on when and where they hit.  Raindrop is an structured identity provider inspired by Twitter's [Snowflake](https://github.com/jtejido/snowflake) where the identity is composed of:
  ...
  Another takeway when reviewing the code is that the construction and comparison of identities is fast e.g. no branching loops.  An identity is struct that lives on the stack and is throw away consuming very little memory or CPU time.
