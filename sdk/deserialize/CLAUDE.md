...
**`Deserialization.DeserializeAsync<T>(PipeReader, DeserializeOptions)` is the first concrete, callable entry point** — resolves `TypeDefinition<T>`, wraps the stream in a fresh `PipeReaderBuffering`, and returns a real `IAsyncEnumerator<Value>` via `ValueAsyncEnumerable<T, PipeReaderBuffering, PipeReader>`. Writing it surfaced that the `Testing*` fixtures (`Forestry.Deserialize.Tests`) were entirely broken — 73 compile errors, none touched since before this whole redesign — so before this could be exercised, they needed fixing:
- `TestingDeserializeOptions`/`TestingPropertyDefinition`/`TestingDeserializerProvider`/`TestingTypeDefinition`/`TestingObjectDeserializer`/`TypeDefinitionReflectionTests` — mechanical namespace fixes (they'd just moved under `Definitions`/`Deserializers`/`Reading`), plus `TestingDeserializeOptions` implementing the new `CreateReaderState<TState>` member (throws — not exercised by any current test) and `TestingObjectDeserializer` stubbing the base `Deserializer.TryReadValue` (also throws — it's only ever driven through schema reflection today, not the pipe walk).
- **`TestingValueDeserializer` rewritten from a non-generic `Deserializer` to `TestingValueDeserializer<T> : Deserializer<T>`** — the old one, keyed by a runtime `Type`, could never produce a real `TypeDefinition<T>` (only the reflection-only `TestingTypeDefinition`), and `Deserialization.DeserializeAsync<T>` does an explicit `(TypeDefinition<T>)` cast that would throw against it. The new one implements `TryReadNullableValue` for real (reads whatever's buffered, parses it, calls `Advance`) and `InitializeTypeDefinition` returning a genuine `new TypeDefinition<T>(this, options)`. Tracks "already produced its one value" on the instance itself, not via `ReaderPath`/`ReaderPosition` — an explicit test-only simplification (a fresh instance is built per test, so it's safe here, but it's not the general pattern a real media `Deserializer<T>` should follow).
- **`TestingReader.cs`, `ValueAsyncEnumerableTests.cs`, `SchemaGuidedReadingTests.cs` deleted, not fixed** — all three exclusively exercised the untyped `IReader`/`ValueAsyncEnumerable` path deleted earlier in this same task. Nothing mechanical to patch; the abstraction they tested no longer exists. Nothing lost — the schema-only assertions `SchemaGuidedReadingTests` also made are already covered by `TypeDefinitionReflectionTests`.

**First real end-to-end proof the walk actually works, not just compiles**: `DeserializationPipeTests.DeserializeAsync_ForAScalarType_ItShould_YieldExactlyOneValue` writes `"hello"` into a real `System.IO.Pipelines.Pipe`, completes the writer, and drives it through `Deserialization.DeserializeAsync<string>` end to end — real `PipeReader` → `PipeReaderBuffering` → `Deserializer<T>.TryReadValue` → `TestingValueDeserializer<T>.TryReadNullableValue` → back out as a `Value` — asserting exactly one value comes out with the right raw content. All 4 runnable tests pass (plus the 1 pre-existing, unrelated `[Fact(Skip)]`).

## Acceptance criteria
- Satisfied, for the core (`Forestry.Deserialize`) package's plumbing: `ReadingStatus` tri-state wired end to end through `Deserializer`/`Deserializer<T>`/`ValueAsyncEnumerator`; `ReaderPath`/`ReaderPosition` real, public, and correctly scoped (ownership corrected mid-build per above); `PipeReaderBuffering` implemented against real `PipeReader` semantics; `Deserialization.DeserializeAsync<T>` wired end to end and proven against a real `Pipe`, not just a fake. `Forestry.Deserialize.Tests` builds and passes clean. All builds clean.
- Not yet satisfied, moving to `Forestry.Deserialize.Xml` next (tracked by new GitHub issues, not yet filed as of this note):
  ...
  - An `Object`-kind (nested properties) end-to-end test through the real pipe path — only the scalar/leaf case has been proven so far; `TestingObjectDeserializer` still only supports schema-reflection tests, not the walk itself.
